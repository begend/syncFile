...
func getFile(w http.ResponseWriter, req *http.Request){
	...
	log.Println(fileName + " " + rootPath + " " + code)
	...
	var f *os.File
	 ...
	 if err != nil {
	  	log.Fatalln("open file failed " + fullFileName)
	 }
	buf, err := ioutil.ReadAll(f)

	if code == "gbk"{
		tmpStr := gbkToUtf8(string(buf))
		buf = []byte(tmpStr)
	}

	resStr :=  base64.StdEncoding.EncodeToString(buf)
	fmt.Fprint(w, resStr)

	defer func(){
		log.Println(rootPath + " " + fileName + " get")
		if e := recover(); e != nil{
			log.Fatal(e)
		}
	}()
